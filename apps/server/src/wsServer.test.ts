...
import { Effect, Exit, Layer, ManagedRuntime, PlatformError, PubSub, Scope, Stream } from "effect";
...
const defaultServerSettingsService: ServerSettingsShape = {
  start: Effect.void,
  ready: Effect.void,
  ...
  readonly open: TerminalManagerShape["open"] = (input: TerminalOpenInput) =>
    Effect.sync(() => {
      ...
      const snapshot: TerminalSessionSnapshot = {
        ...
        terminalId,
        cwd: input.cwd,
        executionEnvironment: input.executionEnvironment ?? { kind: "host" },
        status: "running",
        pid: 4242,
        history: "",
        exitCode: null,
...
describe("WebSocket Server", () => {
  ...
  async function createTestServer(
    ...
    const serverSettingsLayer = Layer.succeed(ServerSettingsService, defaultServerSettingsService);
    ...
    const serverSettingsRuntimeLayer = serverSettingsLayer;
  ...
  it("keeps orchestration domain push behavior for provider runtime events", async () => {
    ...
    emitRuntimeEvent({
      type: "item.completed",
      eventId: asEventId("evt-ws-runtime-message-completed"),
      provider: "codex",
      threadId: asThreadId("thread-1"),
      createdAt: new Date().toISOString(),
      turnId: asTurnId("turn-1"),
